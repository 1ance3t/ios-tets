...
class GesturesPicker {
    ...
    static swipeOnPercentageWithCoordinates(from, to) {
        SCREEN_SIZE = SCREEN_SIZE || driver.getWindowRect();
        const pressOptions = this._getDeviceScreenCoordinatesPicker(
            SCREEN_SIZE,
            from
        );
        const moveToScreenCoordinates = this._getDeviceScreenCoordinatesPicker(
            SCREEN_SIZE,
            to
        );
        this.swipe(pressOptions, moveToScreenCoordinates);
    ...
    static _getDeviceScreenCoordinatesPicker(screenSize, coordinates) {
        return {
            x: coordinates.x,
            y: coordinates.y,
        };
